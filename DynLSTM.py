...
#定义权重
weights={
    'in': tf.Variable(tf.random_normal([datadim,n_hidden],seed=1)),
    'out':tf.Variable(tf.random_normal([n_hidden,numclass],seed=1))
}
biases={
...
cost=tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(logits=pred,labels=labels))#所有样本的loss
loss_eachsample=tf.nn.softmax_cross_entropy_with_logits(logits=pred,labels=labels)#每一个样本的loss
...
with tf.Session() as sess:
    ...
    for i in range(epochs):#每一轮epoch
        ...
        if i % showtime==0 and i !=0:#迭代选样本

            indexall=np.where(loss_allsample_con>afa)
            index=indexall[0]
            x_train_shuffle_select=x_train_shuffle[index,:,:]
            y_train_shuffle_select=y_train_shuffle[index,:]
            totalsteps = int(x_train_shuffle_select.shape[0] / batch_size)
        # else:
        #     totalsteps=int(x_train_shuffle.shape[0] / batch_size)
        #训练模型

        for j in range(totalsteps):
            # if i % showtime==0 and i !=0:
            if i< showtime:
                x_train_batch, y_train_batch = get_random_block_from_data(x_train_shuffle, y_train_shuffle, batch_size,j, numclass)

            else:                                                              #
                x_train_batch, y_train_batch = get_random_block_from_data(x_train_shuffle_select,
                                                                          y_train_shuffle_select, batch_size,
                                                                          j, numclass)

            # else:
            #     x_train_batch, y_train_batch = get_random_block_from_data(x_train_shuffle, y_train_shuffle, batch_size,
            #                                                               j, numclass)
 ...
 \
        ...
        #训练玩后根据model输出每一个样本的loss

        for u in range(totalsteps):
            if i < showtime:
                x_train_batch, y_train_batch = get_random_block_from_data(x_train_shuffle, y_train_shuffle, batch_size,
                                                                          j, numclass)
                ...
                x_train_batch, y_train_batch = get_random_block_from_data(x_train_shuffle_select,
                                                                          y_train_shuffle_select, batch_size,
                                                                          j, numclass)
            loss_eachsample_out = sess.run(loss_eachsample, feed_dict={inputs: x_train_batch, labels: y_train_batch})

            loss_allsample.append(loss_eachsample_out)

        loss_allsample_con=np.concatenate((loss_allsample),axis=0)
        if i>showtime:
            if x_train_shuffle_select.shape[0]<x_train_shuffle.shape[0]*0.05:
                break
    ...
    correct_pre_out_all_con=np.concatenat((correct_pre_out_all),axis=0)
